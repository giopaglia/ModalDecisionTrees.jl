...
results_dir = "./NEVArt/journal-v6-ANT-$(supervised_mode)"
...
const datasets_dict = Dict{String,Vector{Int64}}(
	...
	"sure-ant-v1" => ant_dataset_ids,
	"sure-ant-half-v1" => ant_dataset_ids[ceil(Int64, length(ant_dataset_ids) / 2):end]
)
const datasets_path_dict = Dict{String,String}(
	"sure-v1" => "dataset_1",
	"sure-ant-v1" => "dataset_2",
	"sure-ant-half-v1" => "dataset_2"
)
...
function dataset_function(
	...
	return NEVArtDataset(
		"$(data_dir)/NEVArt/$(datasets_path_dict[dataset_name])";
		...
		for (i_frame, frame) in enumerate(dataset[1])
			...
			elseif keep_only_bands[signals[i_frame+signal_i_offset]] == :auto
				...
				blind_feature_selection_kwparams = (
					...
					export_csv = true,
					cache_description_in = description_savedir,
					# join_plots = [],
				)
