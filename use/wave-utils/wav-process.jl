...
"""
	partitionwav(samples, samplerate; wintime = 0.05, steptime = 0.025, preprocess = [ noise_gate!, normalize! ], preprocess_kwargs = [ (level = 0.01,), (level = 1.0,) ], postprocess = [ normalize!, trim_wav! ], postprocess_kwargs = [ (level = 1.0,), (level = 0.0,) ])
	partitionwav(filepath; kwargs...)
...
function partitionwav(samples::Matrix{T}, samplerate::Real; kwargs...)::Tuple{Vector{Vector{T}}, Real} where T<:AbstractFloat
	partitionwav(merge_channels(samples), samplerate; kwargs...)
end
function partitionwav(samples_and_samplerate::Tuple{Vector{T}, Real}; kwargs...)::Tuple{Vector{Vector{T}}, Real} where T<:AbstractFloat
	partitionwav(samples_and_samplerate...; kwargs...)
end
function partitionwav(filepath::String; kwargs...)::Tuple{Vector{Vector{AbstractFloat}}, Real}
	samples, samplerate = wavread(filepath)
	partitionwav(samples, samplerate; kwargs...)
end
