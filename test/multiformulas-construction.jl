...
@test map(r->count(!isnothing, r) == 1, eachrow(m1))
@test map(r->count(!isnothing, r) == 1, eachrow(m2))

@test map(r->count(!isnothing, r) == 1, eachcol(m1))
@test map(r->count(!isnothing, r) == 1, eachcol(m2))
...
preds2 = filter.(!isnothing, eachrow(hcat(succinct_y...)))
preds2 = first.(preds2)
