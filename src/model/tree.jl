...
module tree
	...
	@inline function fork!(node::NodeMeta)
		...
		node.l = NodeMeta(region[    1:ind], depth, mdepth, l_oura)
		node.r = NodeMeta(region[ind+1:end], depth, mdepth, r_oura)
	end
	...
	function DecisionTree.fit(
			...
			W                       :: AbstractVector{U} = UniformArray{Int}(1,n_samples(Xs)) # the weight vector  # from StructuredArrays
			;
			...
			##########################################################################
			# Modal parameters
			n_subrelations          :: Vector{<:Function},                            # the relations used for the decisions
			n_subfeatures           :: Vector{<:Integer},                             # number of features for the decisions
			initConditions          :: Vector{<:DecisionTree._initCondition},         # world starting conditions
			allowRelationGlob       :: Vector{Bool},                                  # allow/disallow using RelationGlob at any decisional node
			##########################################################################
		...
		function _fit(_perform_consistency_check :: Union{Val{true},Val{false}})
			...
			indX = collect(1:n_instances) # Note TODO clean: an array of sample indices (we split using samples in indX[node.region])
			...
			const NM = begin
				if L<:String
					NodeMeta{Float64,Integer}
				else
					NodeMeta{Float64,Float64}
				end
			end
			root = NM(1:n_instances, 0, 0, onlyallowRelationGlob)
			# Process stack of nodes
			stack = NM[root]
			currently_processed_nodes::Vector{NM} = []
		...
		root, indX = _fit(Val(perform_consistency_check))
