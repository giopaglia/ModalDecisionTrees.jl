...
computeModalThresholdDual(test_operator::_TestOpGeq, w::Interval2D, r::_Topo_EC, channel::MatricialChannel{T,2}) where {T} = begin
	X,Y = size(channel)
	reprs = [
...
computeModalThresholdDual(test_operator::_TestOpGeq, w::Interval2D, r::_Topo_PO, channel::MatricialChannel{T,2}) where {T} = begin
	# if true &&
	...
	# 		# println(computeModalThresholdDual(test_operator, w, _IA2DRel(rx1        , RelationId), channel))
	# 		# println(computeModalThresholdDual(test_operator, w, _IA2DRel(rx2        , RelationId), channel))
	# 		# println(computeModalThresholdDual(test_operator, w, _IA2DRel(RelationId , ry1),        channel))
	# 		# println(computeModalThresholdDual(test_operator, w, _IA2DRel(RelationId , ry2),        channel))
	# 		# println(maxExtrema((
	# 		# 	computeModalThresholdDual(test_operator, w, _IA2DRel(rx1        , RelationId), channel),
	# 		# 	computeModalThresholdDual(test_operator, w, _IA2DRel(rx2        , RelationId), channel),
	# 		# 	computeModalThresholdDual(test_operator, w, _IA2DRel(RelationId , ry1),        channel),
	# 		# 	computeModalThresholdDual(test_operator, w, _IA2DRel(RelationId , ry2),        channel),
	# 		# 	))
	# 		# )
	# 	end
...
computeModalThreshold(test_operator::_TestOpGeq, w::Interval2D, r::_Topo_PO, channel::MatricialChannel{T,2}) where {T} = begin
	...
	# 	println(max(
	# 		computeModalThresholdDual(test_operator, w, _IA2DRel(RelationId , IA_O),       channel),
	# 		computeModalThresholdDual(test_operator, w, _IA2DRel(RelationId , IA_Oi),      channel),
	# 		computeModalThresholdDual(test_operator, w, _IA2DRel(IA_Oi      , RelationId), channel),
	# 		computeModalThresholdDual(test_operator, w, _IA2DRel(IA_O       , RelationId), channel),
	# 	))
	# 	readline()
	# end
	x_singleton = ! (w.x.x < w.x.y-1)
...
computeModalThresholdDual(test_operator::_TestOpGeq, w::Interval2D, r::_Topo_TPP, channel::MatricialChannel{T,2}) where {T} = begin
...
computeModalThresholdDual(test_operator::_TestOpGeq, w::Interval2D, r::_Topo_TPPi, channel::MatricialChannel{T,2}) where {T} = begin
	X,Y = size(channel)
	reprs = [
