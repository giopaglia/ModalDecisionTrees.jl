...
struct ExplicitModalDataset{T<:Number, WorldType<:World} <: ActiveModalDataset{T, WorldType}
    ...
    #  with signature (iC::InitCondition) -> vs::WorldSet{WorldType}
    init_world_sets_funs   :: AbstractVector{<:initWorldSetFunction}
    # Accessibility functions (one per instance)
    #  with signature (w::WorldType/WorldSet{WorldType}, r::Relation) -> vs::AbstractVector{WorldType}
    accessibles_funs      :: AbstractVector{<:accFunction}
    # Representative accessibility functions (one per instance)
    #  with signature (feature::ModalFeature, aggregator::Aggregator, w::WorldType/WorldSet{WorldType}, r::Relation) -> vs::AbstractVector{WorldType}
...
#
abstract type RelationalSupport{T, WorldType} <: AbstractSupport{T, WorldType}     end
...
isminifiable(::Union{AbstractFWD,RelationalSupport,AbstractGlobalSupport}) = true

function minify(fwd_or_support::Union{AbstractFWD,RelationalSupport,AbstractGlobalSupport})
...
struct GenericRelationalSupport{T, WorldType} <: RelationalSupport{T, WorldType}
...
struct ExplicitModalDatasetS{T<:Number, WorldType<:World} <: ExplicitModalDatasetWithSupport{T, WorldType}
    ...
    fwd_rs              :: RelationalSupport{T, WorldType}
...
mutable struct ExplicitModalDatasetSMemo{T<:Number, WorldType<:World} <: ExplicitModalDatasetWithSupport{T, WorldType}
    ...
    # Relational and global support
    fwd_rs              :: RelationalSupport{<:Union{T,Nothing}, WorldType}
...
function modal_step(
    ...
    ) where {T, N, WorldType<:World, WorldSetType<:WorldSet{WorldType}}
    ...
    if returns_survivors isa Val{true}
        worlds_map = Dict{WorldType,WorldSet{WorldType}}()
    end
    if length(worlds) == 0
